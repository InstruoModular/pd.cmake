...
# ╰──────────────────────────────────────╯
if(CMAKE_SYSTEM_PROCESSOR MATCHES "aarch64" AND CMAKE_SYSTEM_NAME MATCHES
                                                "Linux")
...
elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "arm" AND CMAKE_SYSTEM_NAME MATCHES
                                                "Linux")
  set(CMAKE_SYSTEM_NAME Linux)
  ...
  if(ARM_GCC AND ARM_GXX)
    ...
    message(
      FATAL_ERROR "ERROR: Cross-compilers for ARM architecture not found.")
  endif()
endif()
...
if(NOT PD_SOURCES_PATH)
  if(WIN32 OR MINGW)
    ...
    if(NOT PD_HEADER_PATH)
      message(
        FATAL_ERROR
          "<m_pd.h> not found in C:\\Program Files\\Pd\\src, is Pd installed?")
    endif()
  ...
  elseif(APPLE)
    ...
    if(PD_INSTALLER)
      foreach(app ${PD_INSTALLER})
        get_filename_component(PD_SOURCES_PATH "${app}/Contents/Resources/src/"
                               ABSOLUTE)
      endforeach()
    else()
      message(
  ...
  elseif(UNIX AND NOT APPLE)
    ...
    if(NOT PD_HEADER_PATH AND NOT EMSCRIPTEN)
      message(
        WARNING "<m_pd.h> not found in /usr/include/pd/, is Pd installed?")
    endif()

  else()
    message(
      FATAL_ERROR "PD_SOURCES_PATH not set and no default path for the system")
  endif()
...
macro(set_pd_sources PD_SOURCES)
  message(
    DEPRECATION "set_pd_sources is deprecated, use pd_set_sources instead")
...
function(pd_add_datafile OBJ_TARGET DATA_FILE)
  ...
  cmake_parse_arguments(PD_DATAFILE "${BOOLEAN_ARGS}" "${ONE_ARGS}"
                        "${MULTI_ARGS}" ${ARGN})
...
macro(pd_set_lib_ext OBJ_TARGET_NAME)
  ...
  if(PD_EXTENSION)
    set_target_properties(${OBJ_TARGET_NAME} PROPERTIES SUFFIX
                                                        ".${PD_EXTENSION}")
  else()
    if(APPLE)
      set(PD_ETENSION ".pd_darwin")
    elseif(UNIX)
...
function(pd_add_external PD_EXTERNAL_NAME EXTERNAL_SOURCES)
  ...
  set(MULTI_ARGS CXX_FLAGS C_FLAGS LINK_LIBRARIES) # Define multi args,
                                                   # CXX_FLAGS C_FLAGS
  cmake_parse_arguments(PD_EXTERNAL "${BOOLEAN_ARGS}" "${ONE_ARGS}"
                        "${MULTI_ARGS}" ${ARGN})
  ...
  if(${PD_EXTERNAL_NAME} MATCHES "~$" AND "${PD_EXTERNAL_TARGET}" STREQUAL "")
    ...
    if(${ENABLE_TILDE_TARGET_WARNING})
      message(
        WARNING "TARGET for the external ${PD_EXTERNAL_NAME} contains \"~\", "
  ...
  if(EMSCRIPTEN OR PD_BUILD_STATIC_OBJECTS)
    ...
    set_target_properties(${OBJ_TARGET_NAME} PROPERTIES OUTPUT_NAME
                                                        ${PD_EXTERNAL_NAME})
    set_property(GLOBAL APPEND PROPERTY ${PROJECT_NAME}_STATIC_LIBRARIES
                                        ${OBJ_TARGET_NAME})
  else()
  ...
  if(DEFINED PD_EXTERNAL_LINK_LIBRARIES)
    target_link_libraries(${OBJ_TARGET_NAME}
                          PRIVATE ${PD_EXTERNAL_LINK_LIBRARIES})
  endif()
  target_include_directories(${OBJ_TARGET_NAME} PRIVATE ${PD_SOURCES_PATH}
  )# Add Pd Includes
  if(WIN32)
    if(PD_FLOATSIZE EQUAL 64)
...
function(add_pd_external PROJECT_NAME EXTERNAL_NAME EXTERNAL_SOURCES)
  message(
    WARNING "add_pd_external is deprecated, use pd_add_external instead.\n")
  ...
  foreach(OUTPUTCONFIG ${CMAKE_CONFIGURATION_TYPES})
    ...
    set_target_properties(
      ${PROJECT_NAME} PROPERTIES LIBRARY_OUTPUT_DIRECTORY_${OUTPUTCONFIG}
                                 ${CMAKE_BINARY_DIR})
    set_target_properties(
      ${PROJECT_NAME} PROPERTIES RUNTIME_OUTPUT_DIRECTORY_${OUTPUTCONFIG}
                                 ${CMAKE_BINARY_DIR})
    set_target_properties(
      ${PROJECT_NAME} PROPERTIES ARCHIVE_OUTPUT_DIRECTORY_${OUTPUTCONFIG}
                                 ${CMAKE_BINARY_DIR})
  endforeach(OUTPUTCONFIG CMAKE_CONFIGURATION_TYPES)
